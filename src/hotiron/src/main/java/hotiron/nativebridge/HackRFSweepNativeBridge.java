...
import hotiron.core.SweepSamples;
import hotiron.jna.HackrfSweepLibrary;
import hotiron.jna.HackrfSweepLibrary.hackrf_sweep_lib_start__fft_power_callback_callback;
...
//		Native.DEBUG_LOAD	= true;
			...
			int lna_gain, int vga_gain, boolean antennaPowerEnable, boolean antennaLNAEnable)
	{
		if (dataCallback == null)
			throw new IllegalArgumentException("dataCallback");
		SweepSamples.requireValid(num_samples);
		...
		int result = HackrfSweepLibrary.hackrf_sweep_lib_start(callback, freq_min_MHz, freq_max_MHz, fft_bin_width,
				num_samples, lna_gain, vga_gain, antennaPowerEnable ? 1 : 0, antennaLNAEnable ? 1 : 0);
		if (result != 0)
			throw new IllegalStateException("native sweep exited with code " + result);
	}
