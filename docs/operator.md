...
- **Tools column** — radio identity, **MCP**, Restart / Stop / Pause, then a **fixed-height band slot** (FM tuner, TV tuner, NFC sniff, and/or BLE sniff only when that service is in the current window — or pinned while parked / scanning). Auto gain + LNA/VGA stay under the slot. HackRF Settings / Chart options tabs are below. The column is **400 px** wide so switching FM→Wi-Fi does not move the plots.
...
Auto gain, LNA, and VGA stay on the tools column under the band slot. Bias-tee, Antenna LNA, FFT/samples, and CLKOUT stay in the **HackRF Settings** tab.
