...
Windows ships `hotiron.cmd` next to it. The window title is **HotIron**.
...
| **Show peaks** | Extra trace that snaps up to a new high, then falls toward the live sweep. **Peak half-life** (default 15 s) is the time for that gap to shrink by half. `0` follows the live line. Empty hop bins are not treated as a fade. |
| **Persistent display** | Heat overlay of earlier sweeps. **Persistence half-life** (default 30 s) is the time for that glow to fade by half. A pause does not age the overlay. Quick Select, FM↔TV, or Listen/Watch start/stop fades the old glow out in a few hundred milliseconds so it does not sit on the new axis. |
