...
Windows ships `hotiron.cmd` next to it. The window title is **HotIron**. At startup, the window fills the current monitor (minus a margin). Quick Select and the sweep-range readout sit in a banner above the plots. The right-hand tools column is a fixed **400 px** so band tuners do not reflow the graphs. On a shorter display, that column remains scrollable.
...
- **Tools column** — radio identity, **MCP**, Restart / Stop / Pause / **⋯** (FFT bin, samples, bias tee, CLKOUT, About). Then a **fixed-height band slot** (FM / TV / NFC / BLE only when that service is in view, or pinned while parked / scanning). The column is **400 px**. Gain and chart toggles live on the plots.
...
Auto gain, LNA, VGA, and Antenna LNA **+14** sit on the **dB rail** to the left of the graphs. Peaks / Persist / Spurs / Auto dB / Alloc sit on the **chart bar** above the plot. **FFT Auto** is a status-bar pill. Bias-tee, FFT bin, samples, CLKOUT, and About are under **⋯** on the radio strip. Spectrum line width is **1.5 px** (not a setting). The waterfall is always on; its palette follows Auto dB.
