...
Windows ships `hotiron.cmd` next to it. The window title is **HotIron**. At startup, the window fills the current monitor (minus a margin). Quick Select and the sweep-range readout sit in a banner above the plots. The right-hand **Spectrum tools** column is a fixed **520 px** so band tuners do not reflow the graphs. On a shorter display, that column remains scrollable. An **MCP log** at the bottom of the column shows bind, clients, and tool calls.
...
The running process exposes an MCP server so an agent can pull the same sweep the plot is showing, and park **Listen** / **Watch**. It does **not** open a second radio. TCP on **127.0.0.1:8765** is on by default (`make start`). Operator flags:

```bash
make start               # GUI + MCP on 127.0.0.1:8765
./src/hotiron/build/hotiron/hotiron.sh --no-mcp   # GUI only
...
The sidebar **MCP** line (and the status-bar `MCP` field) show whether this process is listening (`127.0.0.1:8765`), how many clients are connected, their `initialize` names, and the last tool they called. **MCP off** means the GUI was started with `--no-mcp`. **MCP failed** means the port is already taken.
...
- **Spectrum** — power vs frequency for the current sweep. During **FM Listen**, this chart switches to the live 4 MS/s parked-IQ spectrum (about ±2 MHz around the tuner LO, with the station at +100 kHz). During **TV Watch**, it switches to the live 16 MS/s parked-IQ spectrum (±8 MHz around the channel center). During **NFC Sniff**, it stays on **12–15 MHz** from the parked 10 MS/s IQ (LO 11.56 MHz). Peak hold, persistence, and the MCP `spectrum_history` ring keep running on that local FFT (same half-life controls as sweep). It returns to the operator sweep range afterward. **Auto gain** (default) sets LNA then VGA so the live peak sits near −28 dBm for this Quick Select. The **dB axis auto-scales** the live band (10 dB pad, 10 dB ticks). Turn **Auto dB** off on the chart bar for a fixed **−100…+20** window. When auto-scale is on, the **waterfall palette** uses the same window so FM/Wi-Fi peaks are not crushed into the blue end of a fixed −90…−25 scale. Auto-scale holds through wobble and bursty peaks, opens a tick only if a signal would clip, and shrinks at most one 10 dB tick every few seconds if that whole stretch stayed quiet. **Drag horizontally** on the plot to zoom that frequency span (the radio retunes; the sweep-range readout follows). **Double-click** or **scroll down** to zoom out one step; **scroll up** zooms in around the cursor. The axis updates immediately; the radio waits ~120 ms after the last zoom/Quick Select so a wheel flick is one retune, and the last sweep stays on screen until the new window’s first full sweep arrives. Quick Select clears the zoom stack. When the view is **wider than a single Quick Select button**, those presets are drawn as labeled vertical bands (FM, WiFi 2, LTE-1, 2m, …). Names sit in the **top header**, same as Wi-Fi / FM channel labels. ITU survey envelopes (HF/VHF/UHF) are lighter. A band that fills the plot is omitted so it does not cover the whole screen. Wi-Fi overlay marks the **occupied 20 MHz** of each US channel (ch N starts at center−10 MHz). On 2.4 GHz those slices overlap every 5 MHz (ch 1 is 2402–2422, ch 2 is 2407–2427, ch 11 is 2452–2472). On 5 GHz the 20 MHz channels do not overlap. The empty stretch after channel 64 is U-NII-2B (5350–5470, weather radar) plus unused 5330–5490, not a wide channel. 1 / 6 / 11 and 36 / 48 / 149 / 165 are brighter. The radio’s 20 MHz interleaved hop is padded ±10 MHz under the requested window so FM 88–108 is actually filled (otherwise 97.3 sits in a 93–98 MHz hole). On **FM**, only stations seen in the live sweep are marked — and only when the view is zoomed to that band (about 30 MHz or less), so a wide survey does not fill the header with unreadable 97.3 tags. Each peak is snapped to the US 200 kHz dial (47 CFR 73.201) and labeled like **97.3**. Confidence rises over a few tenths of a second of repeated hits and decays over about two seconds after the peak drops, so a one-sweep flash is not labeled. Empty channels stay unlabeled. Use a finer FFT bin (100 kHz or less) so adjacent odd-tenths separate.
- **Waterfall** — the same RF range over time (newest at the top), labeled **RF waterfall** on the panel and in the status bar. A **time scale** on the left marks **now**, then 1s / 2s / 5s / … down the history. Hover a row for MHz and age. In **Listen** the top chart stays the live ±2 MHz parked-IQ RF line; the strip **splits** into two waterfalls: **RF · ±2 MHz** (same FFT as the top chart) beside **AUDIO · 0–16 kHz** (demodulated audio, gold badge, dBFS). Leave Listen and it is one **RF waterfall** again. In **Watch** the strip **splits the same way as Listen**: **RF · ±8 MHz** (16 MS/s parked IQ, same scrolling FFT waterfall and live dB window as Listen RF) beside **AUDIO · 0–16 kHz** (AC-3 PCM). Decoded MPEG-2 and the IQ spectrogram stay in the 16:9 TV-tuner preview — they are not the RF waterfall. In **NFC Sniff** it is one **NFC · 12–15 MHz** strip (same RF raster, newest at the top — 13.56 vs 12.71 / 12.96 stay visible over seconds). The **13.56 |IQ|** scope is the small LCD in the NFC sniff sidebar: carrier mixed to baseband, last 500 ms, dashed decode floor at −40 dBFS. Field on is a flat line; Type A polls are notches. Wideband HF hash is rejected. The split stays where it was — Watch does not steal height from the spectrum. Pause freezes both the raster and the ages. History is kept when only LNA/VGA change; a Quick Select or zoom that moves the MHz window still clears it.
- **Status bar** — full-width footer under the waterfall: **HackRF One** (SN / FW; click to pick a radio), Restart / Stop / Pause / ⋯, then plot telemetry (mode, FFT Auto, resolution, bins, rate, peak) and MCP.
- **Banner** — HackRF survey wave **1 MHz–7.25 GHz** (log). Left edge is the radio floor, right edge is the selectable max — there is no 0 Hz gutter. **Services** sit **above** the wave; **HF / VHF / UHF / All** sit **below** (blue / teal / copper / cream so they do not disappear on the gray banner). Chip width and color-coded dividers match that band’s slice. Directly to the **right of the wave**: pan/zoom `◀ − + ▶`, with the live range digits (`2402 – 2472 MHz`) in the free space **above** those buttons. **− / +** expand or shrink the gold window on the survey (same center; span doubles or halves).
- **Spectrum tools** — one band face at a time from Quick Select (**FM**, **V-TV** / **U-TV**, **NFC**, **BLE**), or pinned while parked / scanning. The face stacks vertically and **scrolls** in the space between the Spectrum tools title and the **MCP log**. V-TV is the TV tuner (not FM beside it). The column is **520 px**. Radio session lives in the footer. Gain and chart toggles live on the plots.

The sweep retunes whenever you change a setting.

## Radio identity and Pause

The footer identity is the attached unit, not a boolean “connected” flag:
...
If the HackRF is plugged in (or WSL `usbipd attach`) **after** the window is already open, the sweep starts on its own. **Stop** still owns USB: hot-load will not steal the stick until you press **Restart**.
...
| **FM tuner** | Digital **97.3** on top. One Simpson **SIG** needle packed under the readout: while **Listening** it follows demodulated audio (pre-volume); otherwise it is the Scan S-meter. Below it a 1970s **slide-rule** 88–108 window: drag left for lower MHz, right for higher. **− / +** (Tune) is one 200 kHz channel. **◀◀ / ▶▶** (Seek) jump the next **strong** station from Scan. **Scan** sweeps **88–108 MHz** and pins strong stations. **Listen** parks IQ. |
| **TV tuner** | US ATSC 1.0 channels **2–36** (6 MHz, 47 CFR 73.603). **− / +** Tune skips the FM/aviation gaps. **Scan** leaves Watch, sweeps **V-TV** then **U-TV** (~2.5 s of live sweeps each, after USB is up), scores each 6 MHz brick (occupied vs ATSC 1.0-like pilot), then **qualifies** the strongest UHF ATSC-like hits by parking Watch (~20 s each, max 6) to see which render MPEG-2. The roster lists **picture** vs **RF only** as wrapping channel buttons (they stay inside the column). Seek prefers picture. Occupancy is not a decoded station. Click Scan again to cancel. The waterfall clears when the window hops so VHF history is not remapped onto UHF. Auto-gain waits until Scan finishes so it does not restart USB mid-survey. **Seek** jumps that list (kept across a later Wi‑Fi sweep) plus live parked-IQ hits in the ±8 MHz window. **Watch** parks the HackRF at 16 MS/s with an 8 MHz analog bandwidth. HUD is **WATCH ch N — parked IQ · …**. The strip **splits like Listen**: **RF · ±8 MHz** beside **AUDIO · 0–16 kHz**. **Decoded video** is the 16:9 box **under Watching ch N** — IQ spectrogram until MPEG-2 locks, then the picture. AC-3 on the speakers. Click a **14** header tag to jump to it. Click **Watching ch 14** again to resume the RF sweep. With Auto gain enabled, UHF Watch starts at LNA 40 dB + VGA 22 dB, turns on the HackRF RF amp (+14 dB) for that parked session only, and keeps trimming IF toward ~0.5 RMS without restarting USB; uncheck Auto to use the sliders and the Antenna LNA checkbox. HackRF is 8-bit — indoor VHF (ch 2–6) is often too weak; a strong UHF 14–36 brick is the usual bet. |
| **Radio picker** | Serial of the HackRF to open. *First radio* is libhackrf’s default. |
| **BLE sniff** | Second USB: open the bench nRF J-Link CDC (`/dev/ttyACM0`) and list named Nordic sniffer PDUs (ADV_IND, address, channel, RSSI). Sets the HackRF sweep to **2400–2484 MHz**. Does **not** park the HackRF. Needs the matching sniffer HEX on the DK — [flash recipe](nrf-sniffer.md#flash-recipe). |
...
Auto gain, LNA, VGA, and Antenna LNA **+14** sit on the **dB rail** to the left of the graphs. Peaks / Persist / Spurs / Auto dB / Alloc sit on the **chart bar** above the plot. **FFT Auto** is a footer pill. Bias-tee, FFT bin, samples, CLKOUT, and About are under **⋯** on the footer. Spectrum line width is **1.5 px** (not a setting). The waterfall is always on; its palette follows Auto dB.
...
| WiFi 2 | 2402–2472 | Occupied US 802.11 ch 1–11 (ch 1 starts at 2402, ch 2 at 2407, ch 11 ends at 2472). The 20 MHz channels overlap. BLE advertising 37/38 and data 0–36 sit in this window; **BLE 39 is 2480** (off the right edge). |
| BLE | 2400–2484 | Bluetooth LE ISM so advertising **37 / 38 / 39** are on-screen (39 is 2480). Overlay ticks those three plus ANT+ at 2457. Occupancy may label `BLE 39`. Sidebar **BLE sniff** is the nRF ACM, not a HackRF park. Notes: [nrf-sniffer.md](nrf-sniffer.md). |
...
| WiFi 6 | 5925–7125 | US 6 GHz U-NII-5…8 (47 CFR 15.407). First 20 MHz PSC is ch 1 at 5955 (5945–5965); ch 233 ends at 7125. Overlay ticks those 20 MHz channels. |
| LTE-1 | 1695–2200 | 3GPP AWS + PCS + IMT (B70/B66/B4/B3/B2/B25/B1/B65). |
| LTE-2 | 617–960 | 3GPP 600/700/800/850/900 (B71 DL through B8 DL). |
| n41 | 2496–2690 | 3GPP n41 / B41 TDD 2.5 GHz (US BRS/EBS). LTE-1 stops at 2200; BLE stops at 2484. |
| C-band | 3700–3980 | US C-band 5G (FCC 3.7 GHz auction). 3GPP n77 is 3300–4200; this is the 3700–3980 slice. |
...
| Air | 118–137 | Civil VHF AM airband (47 CFR 87.173). 25 kHz COM; ACARS near 136–137. Inside VHF, but VHF is too wide to see channels. |
| ADS-B | 978–1091 | 1090 ES Mode S plus UAT 978 (ICAO Annex 10 / 47 CFR 87). Tight 1090-only is 1089–1091. |
| GNSS | 1559–1610 | RNSS L1 (GPS / Galileo / BDS / GLONASS). GPS L1 is 1575.42; tight GPS-only is 1574–1577. |
...
| **LNA Gain / VGA Gain** | Vertical faders on the left of the graphs (locked while Auto is on). Raise LNA first if you take over manually. |
...
| **Waterfall palette** | Follows the live dB window (Auto dB). Not a separate pair of sliders. |
