...
HotIron wraps `hackrf_sweep` as a native shared library (via JNA) for high-performance wideband sweeps, parks the same HackRF for FM Listen, ATSC Watch, and NFC Sniff, and exposes the same bins to Grok/Claude/Cursor over localhost MCP (`127.0.0.1:8765`, on by default) without a second USB open.
...
- Supports real-time spectrum, waterfall (left-side time scale), peak/persistent display, spur filter, frequency allocations, quick band selectors, Auto gain, Antenna LNA (+14 dB), and a localhost MCP interface (on by default) with read-only snapshot/diagnostic tools plus explicit FM Listen, TV Watch, NFC Sniff, and parallel nRF BLE sniff controls.
...
**Primary use case**: An operator with a physical HackRF, plus a local MCP client that reads the live sweep and can park Listen/Watch/Sniff.
...
- `make start` — Build (if needed) + launch the Linux app (MCP on `127.0.0.1:8765`)
- `make mcp` — Alias for `start` (MCP already listens on `127.0.0.1:8765`; pass `--no-mcp` to disable)
...
- Plot overlays go through `FrequencyAxis` + `BandMark` + `BandHeaderPainter`. Do not invent a second MHz↔pixel map. The banner survey strip is a different axis (`SpectrumSurveyAxis`, HackRF 1–7250 MHz log) — do not reuse it for the plot. Operator chrome: `OperatorShell` + `OperatorLayout` sizes; Quick Select + sweep range live in `OperatorNavBanner`. Gain is `SpectrumGainRail` left of the plots; chart toggles are `ChartToggleBar`; radio identity + Restart/Stop/Pause/⋯ live on `SweepStatusBar` (full-width footer). FFT/CLKOUT/bias-tee/About are `HardwarePane` under footer ⋯. The east column is **Spectrum tools** (`BandToolsSlot` + `McpLogPane`, 520 px) — do not bring back HackRF Settings / Chart options tabs. Band faces register on `BandToolsSlot` as `BandTool` (`BandToolKind` + view). Visibility policy is `BandToolKind` / `BandContext` (one face at a time; V-TV is TV, not FM beside it) — do not add an if/else in `HackRFSweepSettingsUI` for a new tuner. The slot scrolls above the MCP log. Do not `pack()` the frame or change column width when the sweep range changes. Spectrum line width is `OperatorLayout.SPECTRUM_LINE_WIDTH` (1.5 px), not a setting.
