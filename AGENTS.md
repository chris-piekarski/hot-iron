...
This repository is **HotIron**: a Java 21 HackRF desktop with Sweep / Listen / Watch, live FM/TV station lock, and an in-process MCP server on the same JVM as the GUI. One rig. Agents copy the RF bins. They never open USB.

## Project Overview

HotIron wraps `hackrf_sweep` as a native shared library (via JNA) for high-performance wideband sweeps, parks the same HackRF for FM Listen and ATSC Watch, and exposes the same bins to Grok/Claude/Cursor over localhost MCP (`make mcp`) without a second USB open.
...
**Primary use case**: An operator with a physical HackRF, plus an optional local MCP client that reads the live sweep and can park Listen/Watch.
...
- [docs/nfc.md](docs/nfc.md) — NFC / HF RFID at 13.56 MHz (PHY, sidebands, overlay / `nfc_activity`)
...
- Live agent access is `hotiron.mcp` (`SpectrumSnapshotStore` from `onFullSweepProcessed` and parked-IQ publishers, `SpectrumMcpServer` JSON-RPC). Snapshot/diagnostic tools are read-only (`spectrum_summary`, `spectrum_snapshot`, `radio_identity`, `sweep_config`, `fm_stations`, `nfc_activity`, `fm_spectrum`, `tv_spectrum`, `spectrum_occupancy`, `spectrum_history`, `spectrum_history_bins`, `tv_debug`, `tv_debug_history`); occupancy is `SpectrumOccupancy` on filled bins, not a new USB path. `spectrum_history_bins` is the snapshot ring as filled-bin frames (capped), not the waterfall image. `nfc_activity` is the live 13.56 classifier from those bins (no payload decode, no extra USB). Do not restart USB from a snapshot tool. `sweep_config` reports `radioMode` (`sweep`/`listen`/`watch`/`stopped`). Start with `--mcp` / `make mcp` (localhost 8765). Stdio shim: `scripts/mcp-hotiron-proxy.py`. While Listen/Watch is parked, `spectrum_history` / `spectrum_history_bins` are the local IQ window, not the last wideband sweep.
- FM listen (`WfmDemodulator`, `src-c/hackrf_fm.c`) is exclusive with the wideband sweep: one HackRF, parked 4 MS/s IQ RX, Java mono WFM. The main chart and `fm_spectrum` use an `IqSpectrum` FFT of that same IQ (local ±2 MHz RF) through `DatasetSpectrumPeak` (peak half-life + persistence + history); the waterfall remains 0–16 kHz demodulated audio. Do not time-slice. Demod belongs in `core` with synthetic IQ tests. `AudioSink` fakes the mixer in unit tests.
...
- NFC / HF RFID (13.56 PHY, sidebands at 12.71 / 14.41, HackRF LO offset, what not to decode) live in [docs/nfc.md](docs/nfc.md). Overlay/tracker is `NfcBandPlan` + `NfcActivityTracker` + `NfcBandLayer`. Do not invent a second NFC stack or decode UIDs/APDUs; overlays still go through `FrequencyAxis` + `BandMark`.
...
- Listen and Watch stop the sweep and exclusively park the same HackRF. Watch uses 16 MS/s / 8 MHz and needs host `ffmpeg` for MPEG-2/AC-3; WSL audio needs Pulse/PipeWire forwarded to Windows or playback is silent. Tuner **Scan** leaves Listen/Watch, surveys FM (88–108) or TV (VHF then UHF), and pins those hits as the Seek list. NFC Scan (click a 13.56 header tick) dwells 12–15 then 27.12 / 40.68; it classifies the field, it does not pin a Seek list.
