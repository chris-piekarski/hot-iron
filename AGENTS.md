...
- Plot overlays go through `FrequencyAxis` + `BandMark` + `BandHeaderPainter`. Do not invent a second MHz↔pixel map. The banner survey strip is a different axis (`SpectrumSurveyAxis`, HackRF 1–7250 MHz log) — do not reuse it for the plot. Operator chrome: `OperatorShell` + `OperatorLayout` sizes; Quick Select + sweep range live in `OperatorNavBanner`. Gain is `SpectrumGainRail` left of the plots; chart toggles are `ChartToggleBar`; FFT/CLKOUT/bias-tee/About are `HardwarePane` under radio-strip ⋯ — do not bring back HackRF Settings / Chart options tabs. Band faces register on `BandToolsSlot` as `BandTool` (`BandToolKind` + view). Visibility policy is `BandToolKind` / `BandContext` — do not add an if/else in `HackRFSweepSettingsUI` for a new tuner. Do not `pack()` the frame or change column width when the sweep range changes. Spectrum line width is `OperatorLayout.SPECTRUM_LINE_WIDTH` (1.5 px), not a setting.
